        ...
        <form method="GET" class="mb-4 row g-4 align-items-end">
            <div class="col-md-4">
                ...
                <input type="text" name="search" id="search" class="form-control"
                    placeholder="Nom d'utilisateur ou email" value="{{ request('search') }}">
            </div>
            ...
            <table class="table table-bordered table-hover">
                <thead class="text-white bg-primary">
                    <tr>
                        ...
                        <th scope="col">Prenom de l'utilisateur</th>
                        <th scope="col">Email</th>
                        <th scope="col">Direction de l'utilisateur</th>
                        <th scope="col">Localisation de l'utilisateur</th>
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr>
                            <td class="text-nowrap">{{ $loop->iteration + ($users->currentPage() - 1) * $users->perPage() }}
                            </td>
                            <td class="text-nowrap">{{ $user->mat_ag }}</td>
                            <td class="text-nowrap">{{ $user->nom_ag }}</td>
                            <td class="text-nowrap">{{ $user->pren_ag }}</td>
                            <td class="text-nowrap">{{ $user->email }}</td>
                            <td class="text-nowrap">{{ $user->dir_ag }}</td>
                            <td class="text-nowrap">{{ $user->loc_ag }}</td>
                            <td class="text-nowrap">{{ $user->sta_ag }}</td>
                            <td class="text-nowrap">
                                @forelse ($user->roles as $role)
                                    <span class="badge bg-info"><i class="bi bi-person-badge"></i>
                                        {{ $role->name }}</span>
                                @empty
                            ...
                            <td class="text-nowrap">
                                <a href="{{ route('users.show', $user->id) }}" class="btn btn-outline-success btn-sm"><i
                                        class="bi bi-eye"></i></a>
                                <a href="{{ route('users.edit', $user->id) }}" class="btn btn-outline-primary btn-sm"><i
